...
def main(output_directory, Base_Domain, hgridFileName, *args, **kwargs):
    ...
    #bctypes = [[5, 5, 0, 0], [5, 5, 0, 0]] # for elev and uv bc
...
# ========================== Globals ========================== #
# Main Paths:
RunDir = '/work2/noaa/nos-surge/bkhazaei/work/run_dir/'
# Path to inputs:
Base_Domain = RunDir + '/alaska_domain/mesh/ocsmesh/bearing_sea/outputs/' # '/alaska_domain/mesh/sample_mesh/'
hgridFileName =  'BS_Okhotsk_removed_mesh_GEBCO_v2.2_norton_patch_updatedBC_0_360.gr3' #'BS_mesh_v1.14.gr3' 'BS_Okhotsk_removed_mesh_v1.0_updatedBC_0_360.gr3'
#Base_Domain = '/work2/noaa/nos-surge/bkhazaei/work/run_dir/alaska_domain/mesh/ocsmesh/bearing_sea/outputs/subsets/'
#hgridFileName = 'alaska_final_from_Pacific_v0_updatedBC_0_360.gr3'
# Path to outputs:
SCHISM_RunDir = RunDir+'/alaska_domain/BS_Okhotsk_removed_mesh/'  # /alaska_domain/2d_tide_atm_forc/
case_tag = '3d_GEBCO_v2.2_norton_patch_manning_high' #'3d_tide_20lyr_gebco_v2.2_90day_dt150_ibc_0' #3d_tide_10layer # 3d_tide_atm_forc_10layer
...
VGRID_3DPath = SCHISM_RunDir+'vgrid.in.3D.20layers'
VGRID_LSC2Path = RunDir+'/alaska_domain/3d_runs/vgrid.in.mindov6.LSC2'
...
model_start_date = datetime(2018, 6, 1, 0, 0, 0) #Simulation start date in param.nml
dt = 150.0  # time step in sec
rnday = 30  # simulation length in days

init_opt = 'coldstart'  # initialization option, can be: coldstart, hotstart
atm_forc_opt = 'None'  # forcing option, can be: GFS, HRRR, ERA5, sflux, None

vgrid_type = 'SZ'  # vertical coordinate case (2D, SZ, or LSC2)
N_vert_lyr = 20  # number of vertical layers if 3D

baro_type = 'baroclinic' # options: baroclinic (default), barotropic
rough_type = 'None' # options: drag, mannings_manual, none [drag.gr3, or rough.gr3 or manning.gr3 (default) that corresponds to nchi=0,1,-1] 

heat_exchange_opt=False # heat exchange option (ihconsv = 1)
hycom_elev2D_opt=False  # HYCOM BC for elevation
hycom_UV_opt=False  # HYCOM BC for UV
hycom_TS_opt=False  # HYCOM BC for TS
hycom_adjust2D_opt=False  # HYCOM BC for adjust2D

nhot = 0  # Switch for writing hotstart file

NC_avg = 3600  # average variables in output file every NC_avg seconds
NC_stack = min(720, rnday*24)  # average variables in output file every NC_stack hours
