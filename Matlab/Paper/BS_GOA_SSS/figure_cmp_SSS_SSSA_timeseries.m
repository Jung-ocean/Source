...
for ni = 1:3%length(names)
    ...
    if ni == 1
        pmodel_surf = plot(2019:2022, SSS_surf, '-o', 'Color', colors{ni}, 'LineWidth', 2);
%         pmodel_bot = plot(timenum, SSS_bot, '--', 'Color', colors{ni}, 'LineWidth', 2);
    else
%         p(ni-1) = plot(timenum, SSS, '-o', 'Color', colors{ni}, 'LineWidth', 2);
        p(ni-1) = errorbar(2015:2023, SSS, err, '-o', 'Color', colors{ni}, 'LineWidth', 2);
    end
end
uistack(p(1), 'bottom')
uistack(p(2), 'bottom')
...
for ni = 1:3%length(names)
    ...
    if ni == 1
        pmodel_surf = plot(2019:2022, SSS_surf - mean(SSS_surf), '-o', 'Color', colors{ni}, 'LineWidth', 2);
    else
        p(ni-1) = plot(2015:2023, SSS - mean(SSS), '-o', 'Color', colors{ni}, 'LineWidth', 2);
    end
end
