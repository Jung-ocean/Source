...
for mi = 9:9
    ...
    for di = 1:length(index)
        dindex = index(di);
    
    u_mean = squeeze((u(dindex,:,:)));
    u_mean(u_mean < -1000) = NaN;
    v_mean = squeeze((v(dindex,:,:)));
    ...
    titlestr = datestr(time_vec(dindex,:), 'yyyy-mm-dd');
    title(titlestr, 'FontSize', 25);
    
    saveas(gcf,['geouv_AVISO_', domain_case, '_', titlestr, '.png'])
    end
end