...
switch sat
    ...
    case 'SMOS_Arctic'
        ...
        vstr1 = num2str(version);
        vstr2 = num2str(version, '%02i');
        filepath_sat = ['/data/jungjih/Observations/Satellite_SSS/CEC/Arctic/v', vstr1, '/9day/'];
        filename_sat = ['SMOS_L3_DEBIAS_LOCEAN_AD_', yyyymmdd, '_EASE_Arctic_09d_v', vstr2, '.nc'];
        file_sat = [filepath_sat, filename_sat];
        ...
        if exist(file_sat)
            ...
            lon = lon_sat;
            vari = vari_sat;
            disp(['Loading SMOS Arctic v', vstr2, ' daily SSS ', yyyymmdd]);
        else
            lat = NaN;
            lon = NaN;
            vari = NaN;
