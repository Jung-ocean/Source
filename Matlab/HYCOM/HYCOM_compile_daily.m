%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%       Compile various HYCOM daily data as a one NetCDF file
...
clear; clc
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
filepath = 'D:\Data\Ocean\Model\HYCOM\GLBa0.08_expt90.9_daily\';

yyyy = 2013; % target year
dd = 1;

head = ['archv.', num2str(yyyy), '_', num2char(dd,3), '_00_'];

lon_lim = [400:2000]; % Longitude limits
lat_lim = [1400:2600]; % Latitude limits

% Output file name
compiled_ncname = ['HYCOM_', num2str(yyyy), num2char(dd,3), '.nc'];
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
sshfilename = [head, '2d.nc']; sshfile = [filepath, sshfilename];
...
disp('Temperature...')
tnc = netcdf(tfile); 
temp = tnc{'temperature'}(:, :, lat_lim, lon_lim);
close(tnc);
temp(temp > 1000) = NaN;
