...
f = figure;
set(gcf, 'Position', [1 200 1300 700])
t = tiledlayout(1,2);
t.Padding = 'compact';
...
%     [cs, T] = contourf(x, y, vari, contour_interval, 'LineColor', 'none');
    ...
    if mm_first ~= mm_end
        for mm_atm = mm_first:mm_end
            ...
            rho_air = 1.22;
            ERA5_speed = sqrt(ERA5_uwind.*ERA5_uwind + ERA5_vwind.*ERA5_vwind);
            % Large and Pond, 1981
            Cd = (1.2e-3)*ones(size(ERA5_speed));
            index = find(ERA5_speed >= 11);
            Cd(index) = (0.49+0.065.*ERA5_speed(index))*1e-3;
            
            ERA5_ustress = rho_air.*Cd.*ERA5_speed.*ERA5_uwind;
            ERA5_vstress = rho_air.*Cd.*ERA5_speed.*ERA5_vwind;
            ...
            ERA5_uwind_sum = ERA5_uwind_sum + sum(ERA5_ustress(:,:,tindex),3)';
            ERA5_vwind_sum = ERA5_vwind_sum + sum(ERA5_vstress(:,:,tindex),3)';
        end
    else
    ...
    qscale(1).LineWidth = 2;
    qscale(2).LineWidth = 2;
    tscale = textm(scale_wind_text_lat, scale_wind_text_lon, scale_wind_text, 'Color', color_wind, 'FontSize', text_FS-3);
...
%     t3 = textm(text2_lat+0.1, text2_lon-.4, [title_str], 'FontSize', text_FS);
     t4 = textm(text2_lat, text2_lon+1., [ystr], 'FontSize', text_FS+18);
        
     if yi == 1
         title(t, ['7-day average (', title_str, ')'], 'FontSize', 40)
     end
...
c.FontSize = 12;
asdf
exportgraphics(gcf,'aice_uvice_wind_two_panels.tif','Resolution',300)