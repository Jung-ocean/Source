...
for yi = 1:length(yyyy_all)
    ...
    load(['/data/jungjih/ROMS_BSf/Output/Multi_year/Dsm4/dia/ice_volume/GA/ice_volume_Balance_', region, '_', ystr, '_new.mat']);
...
for yi = 1:length(yyyy_all)
    ...
    p(yi) = plot(movmean(dTdt_cumsum./1e9, day_movmean, 'Endpoints', 'fill'), 'LineWidth', 2, 'Color', colors{yi});
...
for yi = 1:length(yyyy_all)
    ...
    load(['/data/jungjih/ROMS_BSf/Output/Multi_year/Dsm4/dia/ice_volume/GA/ice_volume_Balance_', region, '_', ystr, '_new.mat']);
    p(yi) = plot(movmean(dyn.*dt./1e9, day_movmean, 'Endpoints', 'fill'), 'LineWidth', 2, 'Color', colors{yi});
    xticks([datenum(0,1:7,1)])
    xlim([datenum(0,1,0) datenum(0,8,1)])
    ylim([-8 8])
    yticks(-8:4:8)
...
for yi = 1:length(yyyy_all)
    ...
    load(['/data/jungjih/ROMS_BSf/Output/Multi_year/Dsm4/dia/ice_volume/GA/ice_volume_Balance_', region, '_', ystr, '_new.mat']);
    p(yi) = plot(movmean(thermo.*dt./1e9, day_movmean, 'Endpoints', 'fill'), 'LineWidth', 2, 'Color', colors{yi});
    xticks([datenum(0,1:7,1)])
    xlim([datenum(0,1,0) datenum(0,8,1)])
    ylim([-8 8])
    yticks(-8:4:8)
...
box on
asdf
exportgraphics(gcf,'term_balance_ice_volume.tif','Resolution',300)