...
for mi = 1:2
    ...
    nc = netcdf(filename);
    lon_vec = nc{'longitude'}(:);
    lat_vec = nc{'latitude'}(:);
    dep = nc{'depth'}(:);
    vari = nc{varistr}(:);
    vari_scale_factor = nc{varistr}.scale_factor(:);
    vari_add_offset = nc{varistr}.add_offset(:);
    close(nc)
    
    vari = vari.*vari_scale_factor + vari_add_offset;
    ...
    saveas(gcf, [varistr, '_vertical_PN_MyOcean_' , ystr, mstr, '.png'])
