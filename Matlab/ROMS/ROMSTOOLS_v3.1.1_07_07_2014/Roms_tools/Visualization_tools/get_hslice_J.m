function var=get_hslice_J(fname,gname,vname,level,type);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
% 2D variable
%
  var=squeeze(nc{vname}(:,:));
elseif level>0
%
% Get a sigma level of a 3D variable
%
  var=squeeze(nc{vname}(:,level,:,:));
else
%
...
%
  z=get_depths_J(fname,gname,type);
%
% Read the 3d matrix and do the interpolation
%
  var_sigma=squeeze(nc{vname}(:,:,:));
