...
LON_rho=ncGrid{'lon_rho'}(:,:);
LAT_rho=ncGrid{'lat_rho'}(:,:);
...
ncRiver=netcdf(RiverFile,'nowrite');
nbRiver=length(ncRiver('river'));
t=ncRiver{'river_time'}(:)+2440000;  % time in seconds

Xpos=ncRiver{'river_Xposition'}(:);
Epos=ncRiver{'river_Eposition'}(:);
riv_dir=ncRiver{'river_direction'}(:);
transport=ncRiver{'river_transport'}(:,:);
close(ncRiver);
trans_avg=mean(transport).'; 
...
lat=geomean(LAT_rho(:)); %lat=gmean(LAT_rho(:)); edited by J.Jung
xfac=cos(lat*pi/180);
...
for i=1:nbStar
  ...
  text(LonStar(i,1),LatStar(i,1),ts, 'FontSize', 30)
end
hold off;
