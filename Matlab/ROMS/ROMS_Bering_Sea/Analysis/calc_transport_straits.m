    ...
    for st=1:t_point
        ...
        for mm =  mon_start : mon_end
            ...
            for dd = 1:eomday(yyyy,mm)
                ...
                filename = ['Dsm2_spng_avg_', fstr, foot];
                ...
                disp([num2str(yyyy),num2str(mm,'%02i'),num2str(dd,'%02i'), ' transport ',char(point_name(st)),': ',num2str(total_volume/1e+6)])
                %             disp(['YY/MM = ',num2str(yy),'/',num2char(mm,2),'heat transport ',char(point_name(st)),': ',num2str(total_temp*(4.1*10^6)/1e+15)])

                trans(datanum,st) = total_volume/1e+6;
...
outfile = [filename_output, '_', yts,'.mat'];
save(outfile, 'trans');
