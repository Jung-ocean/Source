%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% Calculate harmonic constants for the purpose of comparison to BSm
%
% J. Jung
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for si = 1:length(stations)
    station = stations{si};
    file_obs = ['/data/jungjih/Observations/Bering_Sea_mooring/figures/uv_1h_', names{si}, '.mat'];
    load(file_obs);
    tindex = find(timenum_1h > datenum(yyyy,mm_start,1) & timenum_1h < datenum(yyyy,mm_end+1,1));
    u_obs = u_obs_1h(:,tindex);
    v_obs = v_obs_1h(:,tindex);
    u_mask = u_obs./u_obs;
    v_mask = v_obs./v_obs;

    file = ['uv_1h_', names{si}, '_ROMS_', ystr, '.mat'];
    load(file);
    u_model = u_model.*u_mask;
    v_model = v_model.*u_mask;

    tindex = find(timenum > datenum(yyyy,mm_start,1) & timenum < datenum(yyyy,mm_end+1,1));
    timenum = timenum(tindex);
    u_tmp1 = u_model(:,tindex);
    v_tmp1 = v_model(:,tindex);
...
save(['harmonic_constants_BSm_ROMS_', ystr, '.mat'], 'stations', 'names', 'constituents', 'timenum', 'depth', 'u', 'ubar', 'ubar_freq', 'ubar_amp', 'ubar_pha', 'ubar_predict', 'v', 'vbar', 'vbar_freq', 'vbar_amp', 'vbar_pha', 'vbar_predict')