function fld3d = interp_to_transect_rel_vort_nc(Seg,iss,time_ind,ncfile,Gdims,dxp,dyp,Fld_intrp_p)
ln_scl=Seg.ln_scl;
lt_scl=Seg.lt_scl;
...
%     Rstart_u=[Seg(iss).iLu(1) Seg(iss).iMu(1) 1 ncfile.recnum(itt)];
    Rstart_u=[Seg(iss).iLu(1) Seg(iss).iMu(1) 1 1];
%     Rstart_v=[Seg(iss).iLv(1) Seg(iss).iMv(1) 1 ncfile.recnum(itt)];
    Rstart_v=[Seg(iss).iLv(1) Seg(iss).iMv(1) 1 1];
