...
switch region
    ...
    case 'Line1'
        Trans_label = 'Line1';
        lonu_f = [-178.7333 -171.5371];
        latu_f = [61.2621 63.5030];
end
...
for iv=1:min(length(Rvarname),5)
    switch RvarDim{iv}
        case {'r2d'}   
            Job(iv).id=batch('interp_to_transect_r2d_nc',1, ...
                {Seg,iss,time_ind_rng,file_i,[Grid.L Grid.M], ...
                Rvarname{iv},Fld_intrp_r2d},'Pool',3);
        case {'u2d'}   
            Job(iv).id=batch('interp_to_transect_u2d_nc',1, ...
                {Seg,iss,time_ind_rng,file_i,[Grid.L Grid.M], ...
        ...
        case {'u3d'}
            Job(iv).id=batch('interp_to_transect_u3d_nc',1, ...
                {Seg,iss,time_ind_rng,file_i,[Grid.L Grid.M Grid.N], ...
                Rvarname{iv},Fld_intrp_u},'Pool',3);
        case {'v3d'}
            Job(iv).id=batch('interp_to_transect_v3d_nc',1, ...
                {Seg,iss,time_ind_rng,file_i,[Grid.L Grid.M Grid.N], ...
                Rvarname{iv},Fld_intrp_v},'Pool',3);
        case {'w3d'}
            Job(iv).id=batch('interp_to_transect_w3d_nc',1, ...
                {Seg,iss,time_ind_rng,file_i,[Grid.L Grid.M Grid.N], ...
                Rvarname{iv},Fld_intrp_w},'Pool',3);
        case {'p3d'}
            Job(iv).id=batch('interp_to_transect_rel_vort_nc',1, ...
                {Seg,iss,time_ind_rng,file_i,[Grid.L Grid.M Grid.N], ...
...
out_file = sprintf('%s_transect_ocean_vars_%s.mat', ...
