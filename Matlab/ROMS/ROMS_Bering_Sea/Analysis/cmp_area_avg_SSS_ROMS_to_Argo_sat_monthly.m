...
if ind_plotpoly == 1
    ...
    pcolor(g.lon_rho, g.lat_rho, g.h.*g.mask_rho./g.mask_rho); shading interp
    contour(g.lon_rho, g.lat_rho, g.h, [50 200 1000], 'k');
    caxis([0 1000])
    c = colorbar;
    c.Title.String = 'm';
...
vari_control_all = zeros(length(yyyy_all)*length(mm_all),1);
vari_sat_all = zeros(num_sat, length(yyyy_all)*length(mm_all));
vari_Argo_all = zeros(length(yyyy_all)*length(mm_all),1);
for yi = 1:length(yyyy_all)
    ...
    filepaths_sat = {filepath_RSS_70, filepath_CEC};

    lons_sat = {'lon', 'lon'};
    lons_360ind = [360, 180];
    lats_sat = {'lat', 'lat'};
    varis_sat = {'sss_smap', 'SSS'};
    titles_sat = {'RSS SMAP L3 SSS v5.3 (70 km)', 'CEC SMOS L3 SSS v9.0'};
    ...
    for mi = 1:length(mm_all)
        ...
        if ~isempty(filename_control)
            ...
            vari_control_all(12*(yi-1) + mi) = sum(vari_control(:).*area(:), 'omitnan')./sum(area(:), 'omitnan');
        else
            vari_control = NaN;
        ...
        mask_Argo_model = (mask_Argo./mask_Argo).*mask;
        area_Argo = area.*mask_Argo_model;
...
ylim(ylimit);
ylabel(unit)
title(['Area averaged SSS'])
l = legend([T1p1, T1ps T1pa], ['ROMS', titles_sat, 'Argo BOA'], 'Interpreter', 'none');
...
p(5) = plot(1:12, vari_control_all(tindex+48), 'LineWidth', 2);
...
for si = 1:num_sat
    ...
    plot(1:12, vari_sat_all(si,tindex+48), 'LineWidth', 2);
...
p(5) = plot(1:12, vari_Argo_all(tindex+48), 'LineWidth', 2);
xticks(1:12);
xlim([0 13])
xlabel('Month')
ylim([32.5 33.5]);
