...
for yi = 1:length(yyyy_all)
    ...
    for mi = 1:length(mm_all)
        ...
        SSS_mean(yi,mi) = mean(salt(index,g.N), 'omitnan');
        v_n_mean(yi,mi) = mean(v_n(index,g.N), 'omitnan');
        str_n_mean(yi,mi) = mean(str_n(index), 'omitnan');
    end
end % yi
...
for yi = 1:length(yyyy_all)
    plot(1:7, str_n_mean(yi,:), '-o', 'LineWidth', 2)
end
ddd
