%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% Plot ROMS zeta and along-track ADT from Satellite monthly
%
% J. Jung
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for li = 1:length(lines)
    ...
    for yi = 1:length(yyyy_all)
        ...
        for mi = 1:length(mm_all)
            ...
            title([direction, 'line ', lstr, ' (', datestr(timenum, 'mmm, yyyy'), ')'], 'FontSize', 15);
...
%             ADT_obs_season(index) = NaN;
            ...
            if ~isempty(jump)
                ...
                for pi = 1:length(index)-1
                    po = plot(lon_line(index(pi)+1:index(pi+1)), ADT_obs_adj(index(pi)+1:index(pi+1)), 'Color', 'r', 'LineWidth', 1);
                    pm = plot(lon_line(index(pi)+1:index(pi+1)), ADT_model_adj(index(pi)+1:index(pi+1)), 'Color', 'k', 'LineWidth', 1);
                end

            else
                po = plot(lon_line, ADT_obs_adj, 'Color', 'r', 'LineWidth', 1);
            ...
            set(gcf, 'Position', [1 1 1000 500])
            pause(3)
            print(['cmp_line_ADT_L2_direct_', direction, 'line_', lstr, '_', datestr(timenum, 'yyyymm')], '-dpng')
        end
