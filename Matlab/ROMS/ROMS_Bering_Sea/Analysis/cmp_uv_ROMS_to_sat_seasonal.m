...
for yi = 1:length(yyyy_all)
    ...
    % Satellite SSH
    % CMEMS
    filepath_CMEMS = ['/data/jungjih/Observations/Satellite_SSH/CMEMS/seasonal/'];
    ...
    for mi = 1:length(month_all)
        ...
        filepattern_control = fullfile(filepath_control,(['*',ystr,'_', month_str,'*.nc']));
...
%         q(2).Color = [.3 .3 .3];
        ...
        filepattern_sat = fullfile(filepath_sat, (['*', ystr, '_', month_str, '*.nc']));
...
%         q(2).Color = [.3 .3 .3];
        ...
        pause(1)
        print(['compare_surface_', vari_str, '_satellite_seasonal_', ystr, '_', num2str(mi, '%02i'), '_', month_str],'-dpng');

        % Make gif
        gifname = ['compare_surface_', vari_str, '_satellite_seasonal.gif'];
