...
SSS_daily = NaN(length(stations), [yeardays(yyyy_all(1))+yeardays(yyyy_all(end))]);
for si = 1:length(stations)
    ...
    for yi = 1:length(yyyy_all)
        ...
        for ti = 1:length(timenum)
            ...
            if ~isempty(tindex)
                SSS_tmp_daily = mean(SSS_tmp(tindex), 'omitnan');
                SSS_daily(si,ti) = SSS_tmp_daily;
            end
        end % ti
    end % yi
end % si

save('SSS_buoy_daily.mat', 'stations', 'ids', 'lats', 'lons', 'timenum', 'SSS_daily')