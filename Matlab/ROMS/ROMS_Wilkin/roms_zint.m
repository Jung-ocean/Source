...
% $Id: roms_zint.m 435 2016-02-09 14:14:44Z wilkin $
...
%
%   Optional inputs to specify vertical integration limits:
%     z_r_indices:
%         If provided, then this is a tuple with the indices
%         into the grd.z_r array.  We wish to integrate between the 
%         two levels specified by these indices.
%     z2, z1:
%         If z1 or z2 are 2-d arrays is it assumed each describes a 
...
%         -h.
%
%         The order of integration is from z2 to z1, so make z2
%         the third argument, and z1 the fourth.
%
%         When z1 or z2 are arrays, dz cannot be returned as output.
%
% Output:
%     vint:
%         Vertical integral.
...
% Simplest case is integrate over full depth. Set the z_r indices
% to cover the entire water column.  We can also compute dz in this case.
if nargin == 2
  INTEGRATE_OVER_ENTIRE_WATER_COLUMN = 1;
end

% Case where we were given the indices into z_r. We can still compute dz 
% since we are given indices into the vertical layers of z_r.
if nargin == 3
  z_r_indices = varargin{1};
  if length(z_r_indices) ~= 2
    error ('length of third argument must be two');
  end
...
if INTEGRATE_OVER_ENTIRE_WATER_COLUMN
  ...
  % We're basically done. Pack up the output arguments and
  switch nargout
    case 2
