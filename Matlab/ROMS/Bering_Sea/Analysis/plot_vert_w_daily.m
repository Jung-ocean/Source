...
for ti = 1:length(timenum_all)
    ...
    [x3, aice_data] = load_BSf_line_2d(g, 'aice', timenum, domaxis);
    ...
    set(gca, 'YColor', 'b')
    p3 = plot(x3, aice_data, '-b', 'LineWidth', 2);
    ylim([0 1]);

    title('Sea level (black) and Sea ice fraction (blue)', 'FontSize', 15)

    ax2 = nexttile(2,[2 1]);
    p2 = plot_contourf([], x, Yi, data*86400, color, climit, contour_interval);
    ...
    title(['Vertical velocity'], 'FontSize', 15);
