...
for yi = 1:length(yyyy_all)
    ...
    AO = load_AO_daily(wtime);
    
    AO = movmean(AO,7, 'Endpoints', 'fill');
...
%     [R, P] = corrcoef(wind_NE(tindex), ALBSA(tindex));
    [R, P, P_ess, ESS] = calc_ess_and_pvalue(wind_NE(tindex),AO(tindex));
    [P_ess, ESS]
    ...
    yyaxis right
    set(gca, 'Ycolor', 'r')
    p2 = plot(wtime(tindex), AO(tindex), '-r', 'LineWidth', 2);
    ylim([-4 4]);
    ylabel('AO');
