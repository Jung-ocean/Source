%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% Compare two ROMS outputs through volume-averaged
%
% J. Jung
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
h = g.h;
dz = g.z_w(2:end,:,:)-g.z_w(1:end-1,:,:);
dz_sum = squeeze(sum(dz,1)).*mask;
dx = 1./g.pm; dy = 1./g.pn;
area = dx.*dy.*mask;
...
for fi = 1:length(filenum_all)
    ...
    vari_control = ncread(file_control,vari_str,[1 1 1 1],[Inf Inf Inf 1]);
    vari_control = permute(vari_control, [3 2 1]);
    vari_control_vmean = squeeze(sum(vari_control.*dz,1))./dz_sum;
    vari_control_shelf(fi) = sum(vari_control_vmean(index_shelf).*area(index_shelf), 'omitnan')./sum(area(index_shelf), 'omitnan');
    vari_control_basin(fi) = sum(vari_control_vmean(index_basin).*area(index_basin), 'omitnan')./sum(area(index_basin), 'omitnan');
    ...
    vari_exp = ncread(file_exp,vari_str,[1 1 1 1],[Inf Inf Inf 1]);
    vari_exp = permute(vari_exp, [3 2 1]);
    vari_exp_vmean = squeeze(sum(vari_exp.*dz,1))./dz_sum;
    vari_exp_shelf(fi) = sum(vari_exp_vmean(index_shelf).*area(index_shelf), 'omitnan')./sum(area(index_shelf), 'omitnan');
    vari_exp_basin(fi) = sum(vari_exp_vmean(index_basin).*area(index_basin), 'omitnan')./sum(area(index_basin), 'omitnan');
...
ylim(ylimit);
ylabel(unit)
title(['Shelf volume averaged (< ', num2str(depth_target), ' m)'])
...
ylim(ylimit);
ylabel(unit)
title(['Basin volume averaged (> ', num2str(depth_target), ' m)'])
l = legend([T2p1, T2p2], case_control, case_exp, 'Interpreter', 'none');
l.Location = 'SouthWest';
l.FontSize = 15;

pause(1)
print(strcat('compare_volume_averaged_', vari_str),'-dpng');