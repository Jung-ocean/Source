...
load('/data/jungjih/ROMS_BSf/Output/Multi_year/Dsm4/SSS/Gulf_of_Anadyr/Fi_ASI_Gulf_of_Anadyr_daily.mat');
...
title('Sea ice fraction (ASI)', 'FontSize', 15)

nexttile(2); hold on; grid on;
load('SSS_SMAP_Gulf_of_Anadyr_common_daily');
for yi = 1:length(yyyy_all)
    yyyy = yyyy_all(yi);
...
ylabel('psu')
set(gca,'FontSize', FS)
title('SSS (SMAP)', 'FontSize', 15)

nexttile(3); hold on; grid on;
load('SSS_SMOS_Gulf_of_Anadyr_common_daily');
for yi = 1:length(yyyy_all)
    yyyy = yyyy_all(yi);
    ...
    p(yi) = plot(timenum_plot(index2), vari(index2), '-o', 'Color', colors{yi}, 'LineWidth', 2);
end
xlim([xlimit])
ylim([27 34])
xticks(timenum_plot(1):10:timenum_plot(end));
...
ylabel('psu')
set(gca,'FontSize', FS)
title('SSS (SMOS CEC)', 'FontSize', 15)

nexttile(4); hold on; grid on;
load('SSS_CMEMS_Gulf_of_Anadyr_common_daily');
for yi = 1:length(yyyy_all)
    yyyy = yyyy_all(yi);
...
ylabel('psu')
set(gca,'FontSize', FS)
title('SSS (CMEMS)', 'FontSize', 15)

nexttile(5); hold on; grid on;
load('SSS_SMOS_BEC_Gulf_of_Anadyr_common_daily');
for yi = 1:length(yyyy_all)
    yyyy = yyyy_all(yi);
...
ylabel('psu')
set(gca,'FontSize', FS)
title('SSS (SMOS BEC)', 'FontSize', 15)

print('aice_SSS_sats', '-dpng')