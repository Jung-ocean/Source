...
ylimit = [datenum(2019,1,1) datenum(2023,6,30)];
climit1 = [-30 30];
climit2 = climit1;
interval = 5;

[color, contour_interval] = get_color('redblue', climit1, interval);
...
lonind = find(lon_plot > xlimit(1) & lon_plot < xlimit(end));
plotm(lat_line(lonind), lon_line(lonind), '-r', 'LineWidth', 3)
title(['ADT (zeta) line'])

% Plot observation
ax1 = nexttile(2); cla; hold on; grid on
...
title('L2 altimetry ADT')

% Plot model
ax2 = nexttile(3); cla; hold on; grid on
lon_diff = diff(lon_plot);
index = find(lon_diff > 1.5*median(lon_diff));
if ~isempty(index)
    pindex = [0; index; length(lon_plot)];
...
print(['cmp_', direction, 'line_', lstr, '_ADT_ROMS_to_L2'], '-dpng');
