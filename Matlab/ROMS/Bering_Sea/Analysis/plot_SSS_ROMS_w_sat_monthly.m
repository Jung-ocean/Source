...
for yi = 1:length(yyyy_all)
    ...
    for mi = 1:length(mm_all)
        ...
        if yi == 1 && mi == 1
            ...
            hold on;
            contourm(lat, lon, g.h, [50 200], 'Color', [.5 .5 .5]);
        else
            delete(T(1));
            delete(h(1));
        end
        ...
        uistack(T(1),'bottom')
        caxis(climit)
        if isscalar(vari_control) ~= 1
            vari_contour = vari_control;
            vari_contour(isnan(vari_contour) == 1) = 1000;
            [C h(1)] = contourm(lat,lon,vari_contour, cinterval, 'Color', 'k');
%             cl = clabelm(C,h(1), 'LabelSpacing', 1000);
%             set(cl, 'BackgroundColor', 'none')
        ...
        for si = 1:num_sat
            ...
            if yi == 1 && mi == 1
                plot_map('Bering', 'mercator', 'l')
                hold on;
                contourm(lat, lon, g.h, [50 200], 'Color', [.7 .7 .7]);
            else
                delete(T(si+1));
                delete(h(si+1));
            end
            ...
            caxis(climit)
            if isscalar(vari_sat_interp) ~= 1
                vari_contour = vari_sat_interp;
                vari_contour(isnan(vari_contour) == 1) = 1000;
                [C h(si+1)] = contourm(lat,lon,vari_contour, cinterval, 'Color', 'k');
%                 cl = clabelm(C,h(si+1));
%                 set(cl, 'BackgroundColor', 'none')
            end
