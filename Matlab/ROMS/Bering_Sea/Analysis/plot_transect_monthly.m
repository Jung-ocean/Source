...
if strcmp(vari_str, 'pden')
    title(t, ['ROMS ', titlestr, ' (interval = ', num2str(interval), ' kg/m^3)'], 'FontSize', 15);
end
...
%     p = pcolor(lon, depth, vari.*mask); shading flat
    p = plot_contourf([], lon, depth, vari.*mask, color, climit, contour_interval);
    if iscontour == 1
...
%         clabel(cs, h, 'Color', 'k', 'FontSize', 15)
    ...
    elseif strcmp(region, 'Navarin_Matthew')
        xlim([min(min(lon)) lon_limit]);
    elseif strcmp(region, 'Cape_Navarin')
        xlim([lon_limit max(max(lon))]);
    end
    ylim(ylimit)
