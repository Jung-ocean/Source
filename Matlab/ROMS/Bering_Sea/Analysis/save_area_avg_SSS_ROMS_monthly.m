...
for yi = 1:length(yyyy_all)
    ...
    for mi = 1:length(mm_all)
        ...
        zeta = load_models_2d_monthly(exp, 'zeta', g.N, yyyy, mm);
        vari = load_models_3d_monthly(exp, vari_str, yyyy, mm);
        vari_surf = load_models_2d_monthly(exp, vari_str, g.N, yyyy, mm);
        vari_bot = load_models_2d_monthly(exp, vari_str, 1, yyyy, mm);
...
if length(mm_all) == 1
    output_filename = ['SSS_ROMS_', exp, '_', region, '_', num2str(mm_all, '%02i'), '.mat'];
else
    output_filename = ['SSS_ROMS_', exp, '_', region, '.mat'];
end
