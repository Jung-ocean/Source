%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% Compare ROMS area averaged SSS to satellite SSS
%
% J. Jung
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if exist('area_target.mat') == 0
figure;
contour(g.lon_rho,g.lat_rho,h, [50 100 200 500], 'k'); shading interp
p = drawpolygon;
...
pcolorm(g.lat_rho, g.lon_rho, area_target)
print('map_SSS', '-dpng')

ti = 1;
vari_con = NaN(1, len_data);
vari_Argo = NaN(1, len_data);
    ...
    for mi = 1:length(mm_all)
        ...
        for di = 1:eomday(yyyy,mm)
            ...
            if exist(file_control) ~= 0
                vari_con_2d = ncread(file_control,vari_str,[1 1 layer 1],[Inf Inf 1 1])';
                vari_con(ti) = sum(area_target(:).*vari_con_2d(:), 'omitnan')./sum(area_target(:), 'omitnan');
            else
                vari_con(ti) = NaN;
            end
            ...
            for si = 1:num_sat
                ...
                if isempty(filename_sat)
                    ...
                    vari_sat_interp = interp2(lon_sat, lat_sat, vari_sat, g.lon_rho, g.lat_rho);
                    vari_sat_all(si,ti) = sum(area_target(:).*vari_sat_interp(:), 'omitnan')./sum(area_target(:), 'omitnan');
                end
...
index = find(vari_sat_all < 1);
vari_sat_all(index) = NaN;
...
p(1) = plot(timenum, vari_con, 'o');
datetick('x', 'mmm, yyyy')
% ylim([32 33.5])
for si = 1:num_sat
    p(si+1) = plot(timenum, vari_sat_all(si,:), 'o');
end
datetick('x', 'yyyy')
l = legend(p, ['ROMS', titles_sat]);
...
figure; hold on; grid on
for i = 1:6
    p(i) = plot(yearly.Time, yearly{:,i}, '-o');
end
datetick('x', 'yyyy')
l = legend(p, ['ROMS', titles_sat]);
l.Location = 'SouthWest';
