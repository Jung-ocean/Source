...
if ( vpoint ==  1 )
    ...
    for i=i1:i2
        ...
        for k=N:-1:lastlevel         % whole cell transport
            sum=sum+v(jj,i,k)*Hz_v(jj,i,k);
            sum_salt=sum_salt+v(jj,i,k)*Hz_v(jj,i,k)*salt_v(jj,i,k);
            %           sum_temp=sum_temp+v(k,jj,i)*Hz_v(k,jj,i)*temp_v(k,jj,i);
        end
        if (lastlevel == N)          % partial cell transport
            partialH=zeta(jj,i)-selectdepth;
            sum=sum+v(jj,i,N)*partialH;
            sum_salt=sum_salt+v(jj,i,N)*partialH*salt_v(jj,i,N);
            %           sum_temp=sum_temp+v(N,jj,i)*partialH*temp_v(N,jj,i);
        elseif (lastlevel ~= 1)
            partialH=z_v(jj,i,k)-selectdepth;
            sum=sum+v(jj,i,k-1)*partialH;
            sum_salt=sum_salt+v(jj,i,k-1)*partialH*salt_v(jj,i,k-1);
...
elseif  ( vpoint == 0 )
    ...
    for j=j1:j2
        ...
        sum_fresh=0;
        for k=N:-1:lastlevel         % whole cell transport
            sum=sum+u(j,ii,k)*Hz_u(j,ii,k);
            sum_salt=sum_salt+u(j,ii,k)*Hz_u(j,ii,k)*salt_u(j,ii,k);
            %           sum_temp=sum_temp+u(k,j,ii)*Hz_u(k,j,ii)*temp_u(k,j,ii);
        end
        if (lastlevel == N)          % partial cell transport
            partialH=zeta(j,ii)-selectdepth;
            sum=sum+u(j,ii,N)*partialH;
            sum_salt =sum_salt +u(j,ii,N)*partialH*salt_u(j,ii,N);
            %            sum_temp=sum_temp+u(N,j,ii)*partialH*temp_u(N,j,ii);
        elseif (lastlevel ~= 1)
            partialH=z_u(j,ii,k)-selectdepth;
            sum=sum+u(j,ii,k-1)*partialH;
            sum_salt =sum_salt +u(j,ii,k-1)*partialH*salt_u(j,ii,k-1);
