function ext_tracers_ini_HYCOM_J(ininame,grdname,seas_datafile,ann_datafile,missval,...
...
%    missval       : FillValue_ of variable in raw netcdf file
...
X=ncseas{'longitude'}(:);% X = X(1,:);
Y=ncseas{'latitude'}(:);% Y = Y(:,1);
Zseas=-ncseas{'depth'}(:);
T=ncseas{'time'}(:).*30;
tlen=length(T);
Nzseas=length(Zseas);
...
if Nz > Nzseas
  ...
  missval=ncann{dataname}.FillValue_(:);
  for k=Nzseas+1:Nz
    if ~isempty(i2)
...
%missval=ncseas{dataname}.FillValue_(:);
if Nz <= Nzseas
  datazgrid=zeros(Nz,M,L);
end
