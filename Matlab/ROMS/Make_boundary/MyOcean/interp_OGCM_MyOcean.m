...
% Open the OGCM file
%
nc = netcdf([OGCM_dir,OGCM_prefix,'zos_',num2str(year),'_',num2char(month,2),'.nc']);
...
%
zeta = ext_data_OGCM_MyOcean(nc,lonT,latT,'zos',tin,lon,lat,1,missvalue,Roa,interp_method);
close(nc)
%
...
for k = 1:NZ
    ...
    nc = netcdf([OGCM_dir,OGCM_prefix,'uo_',num2str(year),'_',num2char(month,2),'.nc']);
    u2d = ext_data_OGCM_MyOcean(nc,lonU,latU,'uo',tin,lon,lat,...
        k,missvalue,Roa,interp_method);
    close(nc)
    
    nc = netcdf([OGCM_dir,OGCM_prefix,'vo_',num2str(year),'_',num2char(month,2),'.nc']);
    v2d = ext_data_OGCM_MyOcean(nc,lonV,latV,'vo',tin,lon,lat,...
        k,missvalue,Roa,interp_method);
    close(nc)
    ...
    nc = netcdf([OGCM_dir,OGCM_prefix,'thetao_',num2str(year),'_',num2char(month,2),'.nc']);
    temp(k,:,:) = ext_data_OGCM_MyOcean(nc,lonT,latT,'thetao',tin,lon,lat,...
        k,missvalue,Roa,interp_method);
    close(nc)
    
    nc = netcdf([OGCM_dir,OGCM_prefix,'so_',num2str(year),'_',num2char(month,2),'.nc']);
    salt(k,:,:) = ext_data_OGCM_MyOcean(nc,lonT,latT,'so',tin,lon,lat,...
        k,missvalue,Roa,interp_method);
    close(nc)
