function interp_OGCM_SODA3(OGCM_dir,OGCM_prefix,year,month,Roa,interp_method,...
...
disp(['  Horizontal interpolation: ',...
    'SODA3 ','Y',num2str(year),'M',num2str(month),'.nc'])
%
%
% ROMS grid angle
%
...
%
zeta = ext_data_OGCM_SODA3(nc,lonT,latT,'ssh',tin,lon,lat,1,missvalue,Roa,interp_method);
%
...
for k = 1:NZ
    ...
    u2d = ext_data_OGCM_SODA3(nc,lonU,latU,'u',tin,lon,lat,...
        k,missvalue,Roa,interp_method);
    v2d = ext_data_OGCM_SODA3(nc,lonV,latV,'v',tin,lon,lat,...
        k,missvalue,Roa,interp_method);
    u(k,:,:) = rho2u_2d(u2d.*cosa+v2d.*sina);
    v(k,:,:) = rho2v_2d(v2d.*cosa-u2d.*sina);
    temp(k,:,:) = ext_data_OGCM_SODA3(nc,lonT,latT,'temp',tin,lon,lat,...
        k,missvalue,Roa,interp_method);
    salt(k,:,:) = ext_data_OGCM_SODA3(nc,lonT,latT,'salt',tin,lon,lat,...
        k,missvalue,Roa,interp_method); 
end
