function interp_OGCM_GLORYS(OGCM_dir,OGCM_prefix,year,month,Roa,interp_method,...
...
% Open the OGCM file
%
nc = netcdf([OGCM_dir,OGCM_prefix, '_',num2char(month,2),'.nc']);
missvalue = nc{'zos'}.FillValue_(:);
%
...
%
zeta = ext_data_OGCM_GLORYS(nc,lonT,latT,'zos',tin,lon,lat,1,missvalue,Roa,interp_method);
close(nc)
%
...
for k = 1:NZ
    ...
    nc = netcdf([OGCM_dir,OGCM_prefix, '_',num2char(month,2),'.nc']);
    u2d = ext_data_OGCM_GLORYS(nc,lonU,latU,'uo',tin,lon,lat,...
        k,missvalue,Roa,interp_method);
    close(nc)
    
    nc = netcdf([OGCM_dir,OGCM_prefix, '_',num2char(month,2),'.nc']);
    v2d = ext_data_OGCM_GLORYS(nc,lonV,latV,'vo',tin,lon,lat,...
        k,missvalue,Roa,interp_method);
    close(nc)
    ...
    nc = netcdf([OGCM_dir,OGCM_prefix, '_',num2char(month,2),'.nc']);
    temp(k,:,:) = ext_data_OGCM_GLORYS(nc,lonT,latT,'thetao',tin,lon,lat,...
        k,missvalue,Roa,interp_method);
    close(nc)
    
    nc = netcdf([OGCM_dir,OGCM_prefix, '_',num2char(month,2),'.nc']);
    salt(k,:,:) = ext_data_OGCM_GLORYS(nc,lonT,latT,'so',tin,lon,lat,...
        k,missvalue,Roa,interp_method);
    close(nc)
...
if ~isempty(nc_bry)
    ...
    if obc(1) == 1
        [u_south,v_south,...
            ...
            temp_south,salt_south]=vinterp_OGCM_GLORYS(zr(:,1,:),zu(:,1,:),zv(:,1,:),...
            dzr(:,1,:),dzu(:,1,:),dzv(:,1,:),...
            u_south,v_south,...
            ubar_south,vbar_south,...
    ...
    if obc(2) == 1
        [u_east,v_east,...
            ubar_east,vbar_east,...
            temp_east,salt_east]=vinterp_OGCM_GLORYS(zr(:,:,end),zu(:,:,end),zv(:,:,end),...
    ...
    if obc(3) == 1
        [u_north,v_north,...
            ...
            temp_north,salt_north]=vinterp_OGCM_GLORYS(zr(:,end,:),zu(:,end,:),zv(:,end,:),...
            dzr(:,end,:),dzu(:,end,:),dzv(:,end,:),...
            u_north,v_north,...
            ubar_north,vbar_north,...
    ...
    if obc(4) == 1
        [u_west,v_west,...
            ubar_west,vbar_west,...
            temp_west,salt_west]=vinterp_OGCM_GLORYS(zr(:,:,1),zu(:,:,1),zv(:,:,1),...
            dzr(:,:,1),dzu(:,:,1),dzv(:,:,1),...
            u_west,v_west,...
            ubar_west,vbar_west,...
            temp_west,salt_west,...
