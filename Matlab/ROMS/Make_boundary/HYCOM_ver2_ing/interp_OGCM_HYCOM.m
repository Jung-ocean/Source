function interp_OGCM_HYCOM(OGCM_dir,OGCM_prefix,year,month,Roa,interp_method,...
...
% Open the OGCM file
%
nc = netcdf([OGCM_dir,OGCM_prefix,num2str(year),num2char(month,2),'.nc']);
if sum(sum(isnan(nc{'ssh'}(:)))) ~= 0
    missvalue = NaN;
else
    missvalue = max(max(nc{'ssh'}(:)));
end
%
...
%
zeta = ext_data_OGCM_HYCOM(nc,lonT,latT,'ssh',tin,lon,lat,1,missvalue,Roa,interp_method, obc);
%
...
for k = 1:NZ
    ...
    u2d = ext_data_OGCM_HYCOM(nc,lonU,latU,'u',tin,lon,lat,...
        k,missvalue,Roa,interp_method);
    v2d = ext_data_OGCM_HYCOM(nc,lonV,latV,'v',tin,lon,lat,...
        k,missvalue,Roa,interp_method);
    u(k,:,:) = rho2u_2d(u2d.*cosa+v2d.*sina);
    v(k,:,:) = rho2v_2d(v2d.*cosa-u2d.*sina);
    temp(k,:,:) = ext_data_OGCM_HYCOM(nc,lonT,latT,'temp',tin,lon,lat,...
        k,missvalue,Roa,interp_method);
    salt(k,:,:) = ext_data_OGCM_HYCOM(nc,lonT,latT,'salt',tin,lon,lat,...
        k,missvalue,Roa,interp_method);
...
if ~isempty(nc_bry)
    ...
    if obc(1) == 1
        [u_south,v_south,...
            ...
            temp_south,salt_south]=vinterp_OGCM_HYCOM(zr(:,1,:),zu(:,1,:),zv(:,1,:),...
            dzr(:,1,:),dzu(:,1,:),dzv(:,1,:),...
            u_south,v_south,...
            ubar_south,vbar_south,...
    ...
    if obc(2) == 1
        [u_east,v_east,...
            ubar_east,vbar_east,...
            temp_east,salt_east]=vinterp_OGCM_HYCOM(zr(:,:,end),zu(:,:,end),zv(:,:,end),...
    ...
    if obc(3) == 1
        [u_north,v_north,...
            ...
            temp_north,salt_north]=vinterp_OGCM_HYCOM(zr(:,end,:),zu(:,end,:),zv(:,end,:),...
            dzr(:,end,:),dzu(:,end,:),dzv(:,end,:),...
            u_north,v_north,...
            ubar_north,vbar_north,...
    ...
    if obc(4) == 1
        [u_west,v_west,...
            ubar_west,vbar_west,...
            temp_west,salt_west]=vinterp_OGCM_HYCOM(zr(:,:,1),zu(:,:,1),zv(:,:,1),...
            dzr(:,:,1),dzu(:,:,1),dzv(:,:,1),...
            u_west,v_west,...
            ubar_west,vbar_west,...
            temp_west,salt_west,...
