function data=ext_data_OGCM_HYCOM(nc,X,Y,vname,tndx,lon,lat,k,missvalue,Roa,interp_method)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
%if ~isempty(i2)
    if ndims == 2
        ...
        data = squeeze(nc{vname}(:, :));
    elseif ndims == 3
        %data = squeeze(nc{vname}(k,j,i2)); % HYCOM
        data = squeeze(nc{vname}(k,:, :)); % HYCOM
    elseif ndims==4
        %data = squeeze(nc{vname}(tndx,k,j,i2));
        data = squeeze(nc{vname}(tndx,k,:, :));
    else
...
if interp_flag==0
    ...
    data = griddata(x,y,data,lon,lat,'nearest'); % J
else
    %data = interp2(x,y,data,lon,lat,interp_method);
    data = griddata(x,y,data,lon,lat,interp_method); % J
end
