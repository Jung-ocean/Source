...
%-- case default: dx = dy = 1
dx = 0.5; dy = 0.1;
% dx = 1; dy = 1;
ix = max(floor(1/dx),1); iy = max(floor(1/dy),1);
e = 2; g = 1;

[X,Y] = ndgrid(0:dx:20,0:dy:15);
u = e*X - g*Y;
v = g*X - e*Y;
[~, psi] = flowfun((u./dx)',(v./dy)'); psi = psi';
[phi, ~] = flowfun((u.*dx)',(v.*dy)'); phi = phi';

fig1 = figure('name','ndgrid'); hold on; box on; axis equal;
contour(X,Y,phi,20,'--r','linewidth',2);
contour(X,Y,psi,20,'-g','linewidth',2);
quiver(X(1:ix:end,1:iy:end),Y(1:ix:end,1:iy:end),u(1:ix:end,1:iy:end),v(1:ix:end,1:iy:end),'k','linewidth',2);
legend('\Phi','\Psi','interpreter','tex'); xlim([0 20]); ylim([0 15]);

[X,Y] = meshgrid(0:dx:20,0:dy:15);
u = e*X - g*Y;
v = g*X - e*Y;
[~, psi] = flowfun(u./dx,v./dy);
[phi, ~] = flowfun(u.*dx,v.*dy);

fig2 = figure('name','meshgrid'); hold on; box on; axis equal;
contour(X,Y,phi,20,'--r','linewidth',2);
contour(X,Y,psi,20,'-g','linewidth',2);
quiver(X(1:iy:end,1:ix:end),Y(1:iy:end,1:ix:end),u(1:iy:end,1:ix:end),v(1:iy:end,1:ix:end),'k','linewidth',2);
legend('\Phi','\Psi','interpreter','tex'); xlim([0 20]); ylim([0 15]);
pause(0.3);
set(fig1,'position',[0 0 600 400]);
set(fig2,'position',[0 0 600 400]);
