function [x, Yi, data] = load_models_vertical_monthly(model, g, vari_str, yyyy, mm, domaxis)
...
switch model
    ...
    case 'NANOOS'
        ...
        mstr = num2str(mm, '%02i');
        yyyymm = [ystr, mstr];
        filename = ['NANOOS_monthly_', yyyymm, '.nc'];
        file = [filepath, filename];
end
...
if strcmp(vari_str, 'v_n')
    ...
    vari = v_n;
elseif strcmp(vari_str, 'pres')
    lat_3d = repmat(g.lat_rho, [1 1 g.N]);
    pres = gsw_p_from_z(depth,lat_3d);
    vari = pres;
elseif strcmp(vari_str, 'pden')
    temp_sigma = ncread(file, 'temp');
    salt_sigma = ncread(file, 'salt');
