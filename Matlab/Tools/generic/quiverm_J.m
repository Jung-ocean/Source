function h = quiverm_J(lat, lon, u, v, varargin)
...
%%%
% Constants for arrow head size
% Set fixed arrow head properties
alpha = 0.2;   % Fixed length of arrow head relative to vector length
beta = 0.2;    % Fixed width of the base of arrow head

% Adjust width for zonal vectors by cosd(lat)
adjusted_beta_u = beta * cosd(lat);  % Adjust beta by cos(latitude) for zonal direction only
adjusted_beta_v = beta;              % No adjustment for meridional direction

% Calculate perpendicular components to ensure symmetry
perp_u = -v;   % Perpendicular to u
perp_v = u;    % Perpendicular to v

% Normalize perpendicular components
perp_magnitude = sqrt(perp_u.^2 + perp_v.^2);
perp_u = perp_u ./ perp_magnitude;
perp_v = perp_v ./ perp_magnitude;
%%%
...
%             lon+v-alpha*(v+beta*(u+eps))];       hv(4,:) = NaN;
    ...
    %%%
    % Define arrow head coordinates with adjusted width for zonal direction
    hu = [lat + u - alpha * u + adjusted_beta_u .* perp_u; lat + u; lat + u - alpha * u - adjusted_beta_u .* perp_u];
    hu(4, :) = NaN;

    hv = [lon + v - alpha * v + adjusted_beta_v .* perp_v; lon + v; lon + v - alpha * v - adjusted_beta_v .* perp_v];
    hv(4, :) = NaN;
    %%%
