...
filepath = '/data/jungjih/Observations/Sea_ice/ICESat2/ATL07/data/';
...
for mi = 1:length(mm_all)
    ...
    filename_all = dir([filepath, 'ATL07-01_', ystr, mstr, dstr, '*']);

for fi = 1:length(filename_all)
    filename = filename_all(fi).name;
    file = [filepath, filename];

    for gti = 1:length(ground_tracks)
        ground_track = ground_tracks{gti};

        lat = h5read(file, ['/', ground_track, '/sea_ice_segments/latitude']);
        lon = h5read(file, ['/', ground_track, '/sea_ice_segments/longitude']);

        mss = h5read(file, ['/', ground_track, '/sea_ice_segments/geophysical/height_segment_mss']); % m
        geoid = h5read(file, ['/', ground_track, '/sea_ice_segments/geophysical/height_segment_geoid']); % m

        height = h5read(file, ['/', ground_track, '/sea_ice_segments/heights/height_segment_height']); % m
        type = h5read(file, ['/', ground_track, '/sea_ice_segments/heights/height_segment_type']);
