function [uf,tf]=OSUlpFAST(u,t);
%
% [uf,tf]=OSUlpFAST(u,t) ...
% remake of OSUlp, OSUlpAK to get advantage of sparse matrix multiplication 
%                        ...
...
n=length(tf);
ui=interp1(t,u1',tf,'linear'); % [n times nv]
...
%% OLD implementation, OSUlpAK:
%% Lowpass first 60 hours
% for i=1:60;
%  ww=w(60-i+1:121); ww=ww/sum(ww);
%  uf(:,i)=sum(repmat(ww,[nv 1]).*ui(:,1:i+60+1),2); 
%end;
%
...
% NEW IMPLEMENTATION, MUCH FASTER: 
A=zeros(n);
for i=1:60
