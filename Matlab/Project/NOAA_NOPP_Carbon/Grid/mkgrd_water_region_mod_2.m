...
%  jn=findin(lat_rho(1,:),[coor(2) coor(2)+coor(4)]);
  mask_rho1(intmp)=1;
  delete(P);
  pln=plot(lon_rho(intmp),lat_rho(intmp),'bs','markersize',mrksize);
  set(hwatrreg,'Value',0);
 else
  fprintf('Inundate Region cancelled /n')
  delete(P);
  set(hwatrreg,'Value',0); 
 end
end