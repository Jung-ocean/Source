...
file_ROMS = [vari_str, '_ROMS_', region, '.mat'];
if exist(file_ROMS)
    load(file_ROMS)
else
    vari_ROMS = [];
    for ti = 1:length(timenum_obs)
        ...
        profile = load_models_profile_daily('Oregon_1km', g, timenum_target, lon_obs, lat_obs);
        vari_tmp = eval(['profile.', vari_model]);
        vari_ROMS(:,ti) = interp1(profile.depth, vari_tmp, depth);
    end
    save(file_ROMS, 'vari_ROMS')
end
...
plot_contourf(ax2, timenum_obs, depth, vari_ROMS.*scale+offset, color, climit, contour_interval);
colormap(ax2, color);
xlim([timenum_obs(1)-1 timenum_obs(end)+1]);
ylim([min(depth)-2 0])
xticks(datenum(yyyy,[1 4 7 10],1))
...
plot_contourf(ax3, timenum_obs, depth, (vari_ROMS-vari_obs).*scale+offset, color2, climit2, contour_interval2);
colormap(ax3, color2)
xlim([timenum_obs(1)-1 timenum_obs(end)+1]);
ylim([min(depth)-2 0])
xticks(datenum(yyyy,[1 4 7 10],1))
