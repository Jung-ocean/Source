...
filename = 'soda3.4.2_mn_ocean_reg_2013.nc';
...
depth = repmat(dep, [1, length(lat_vec), length(lon_vec)]);

for mi = 1:12
    mm = mi; mstr = num2char(mm,2);
    
    data = squeeze(temp(mm,:,:,:));
    ...
    set(get(bar,'title'),'string','^oC','FontSize',17,'fontweight','bold')
    
    saveas(gcf, ['temp_vertical_PN_SODA3_2013', mstr, '.png'])
