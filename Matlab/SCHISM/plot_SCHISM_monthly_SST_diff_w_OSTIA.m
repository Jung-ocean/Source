...
for mi = 1:length(mm_all)
    ...
    title(t, ['Monthly SST difference (', ystr, mstr, ')'], 'FontSize', 25)
    ...
    nexttile(1);
    pcolor(lon_OSTIA2, lat_OSTIA2, vari_OSTIA_sub - vari_OSTIA_sub); shading interp
    colormap redblue
    ...
    for si = 1:length(SCHISM_expnames)
        ...
        nexttile(si+1)
        pcolor(lon_OSTIA2, lat_OSTIA2, vari_SCHISM_interp - vari_OSTIA_sub); shading interp
        colormap redblue
        xlim([min(Mobj.lon) max(Mobj.lon)])
        ylim([min(Mobj.lat) max(Mobj.lat)])
        caxis(climit)
...
print('monthly_SST_diff_w_OSTIA', '-dpng')