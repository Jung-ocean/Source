...
for i = 1:exp_step
    ...
    ot = tnc{'ocean_time'}(:);
    % bda
    bda_temp = squeeze(mean(cnc{'temp'}(2:end, depth_ind, :, :))) - tnc{'temp'}(1, depth_ind, :, :).*g.mask_rho./g.mask_rho;
    bda_temp_area = bda_temp(rmse_index_all2);
    bda_temp_area(isnan(bda_temp_area)) = [];
    bda_rms_all = [bda_rms_all; ot, rms(bda_temp_area)];
    
    % ada
    ada_temp = squeeze(mean(anc{'temp'}(2:end, depth_ind, :, :))) - tnc{'temp'}(1, depth_ind, :, :).*g.mask_rho./g.mask_rho;
...
save rms_1_daily.mat ada_rms_all bda_rms_all err_diff_sum_all